...
type HeatmapFilter = 'ALL' | 'PÉRDIDA' | 'RECUPERO' | 'FALTA';
...
const LiveGameView: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  // Carga inicial y reconexión de estado persistente
  useEffect(() => {
    if (id) {
  ...
  // Sincronizar estado con DB Local cada vez que el objeto game cambia
  useEffect(() => {
    if (game) {
  ...
  const showPassSnackbar = (count: number) => {
    setSnackbar({ message: `Cadena interrumpida: ${count} pases.`, visible: true });
  ...
  const handleFieldClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (isDouble) {
      ...
      registerEvent('FALTA', Possession.AWAY, x, y, "Doble Tap", game.teamHome.id);
    ...
    const isInTopGoal = y < 15 && x > 35 && x < 65;
    const isInBottomGoal = y > 85 && x > 35 && x < 65;
  ...
  const registerEvent = (type: string, nextPoss: Possession, x: number, y: number, details?: string, forcedTeamId?: string, scoringTeam?: Possession) => {
    ...
    if (type.includes('FALTA')) {
      ...
      if (passCount > 0) {
        ...
        showPassSnackbar(passCount);
        finalDetails = `${finalDetails}${finalDetails ? ' | ' : ''}(${passCount} pases)`;
      }
      setPassCount(0);
    }

    if (type.includes('GOL') || type.includes('DESVIADO')) {
    ...
    const event: GameEvent = {
      ...
      timestamp: Date.now(),
      gameTime: formatTime(seconds),
      type,
      teamId: forcedTeamId || (finalNewPoss === Possession.HOME ? game.teamHome.id : (finalNewPoss === Possession.AWAY ? game.teamAway.id : game.teamHome.id)),
      x: Math.round(x),
      y: Math.round(y),
  ...
  const deleteEvent = (eventId: string) => {
    ...
    if (eventToDelete.type.includes('GOL')) {
      if (eventToDelete.y < 30) updatedScoreHome--;
  ...
  const handleVoiceNote = () => {
    ...
    setTimeout(() => {
      ...
      registerEvent('NOTA VOZ', Possession.NONE, 50, 50, "Audio guardado", game.teamHome.id);
      setSnackbar({ message: "Nota de voz lista.", visible: true });
    ...
    <div className="h-screen w-full flex flex-col bg-surface overflow-hidden select-none transition-all duration-500">
          ...
          <aside className="relative w-72 h-full bg-white border-r border-surfaceVariant shadow-2xl flex flex-col p-8 animate-in slide-in-from-left duration-300">
            ...
            <nav className="flex-1 space-y-2 overflow-y-auto no-scrollbar">
              ...
              {[
                ...
                <button 
                  key={i} 
                  className="w-full text-left p-4 rounded-xl hover:bg-surface text-onSurface font-bold text-[11px] uppercase tracking-widest transition-all active:scale-95"
            ...
            <div className="pt-6 mt-6 border-t border-surfaceVariant">
              ...
              >
                <span>🏠</span> 6: Regresar al dashboard
              </button>
            </div>
          </aside>
        </div>
      ...
      <header className="h-16 md:h-20 flex items-center justify-between px-4 md:px-6 bg-white shrink-0 border-b border-surfaceVariant shadow-sm z-[200]">
        ...
        <div className="flex flex-col items-center shrink-0">
          <div className="bg-surface px-3 py-1 rounded-xl border border-surfaceVariant flex items-center gap-3 mb-0.5">
            <p className="text-lg md:text-xl font-black text-primary tabular-nums leading-none">{formatTime(seconds)}</p>
            <button onClick={() => setIsRunning(!isRunning)} className="w-6 h-6 rounded-full bg-primary text-white flex items-center justify-center active:scale-90 transition-all text-[10px]">
              {isRunning ? '||' : '▶'}
            </button>
          </div>
          <div className="flex gap-2">
            {[1, 2, 3, 4].map(q => (
              <button 
                key={q} 
                onClick={() => setPeriod(q)}
                className={`text-[9px] font-black transition-all ${period === q ? 'text-primary scale-110 drop-shadow-sm' : 'text-onSurfaceVariant/40'}`}
              >
                {q}Q
              </button>
            ))}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar no-scrollbar">
              {game.events.slice().reverse().map((e) => (
                <div key={e.id} className="bg-surface/50 border border-surfaceVariant p-3 rounded-2xl flex items-center justify-between group hover:bg-white transition-all shadow-sm">
        ...
        <div className="flex-1 relative p-2 flex gap-2 overflow-hidden">
          ...
          <div className="flex-1 relative overflow-hidden">
            {activeView === 'heatmap' ? (
              <div className="w-full h-full bg-white rounded-[32px] border-2 border-surfaceVariant flex flex-col p-6 animate-in slide-in-from-bottom duration-300 overflow-hidden shadow-xl">
                ...
                <div className="flex-1 relative overflow-hidden rounded-[24px]">
                  ...
                  <div className="absolute inset-0">
                    {game.events.filter(e => heatmapFilter === 'ALL' || e.type.includes(heatmapFilter)).map((e, i) => (
              ...
              <div className="w-full h-full bg-white rounded-[32px] border-2 border-surfaceVariant flex flex-col p-6 animate-in slide-in-from-bottom duration-300 overflow-hidden shadow-xl">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-sm font-black text-dark uppercase tracking-widest">Eventos Detallados</h3>
                  <button onClick={() => setActiveView('field')} className="text-[10px] font-black text-primary uppercase tracking-widest bg-primary/5 px-4 py-2 rounded-full">Cerrar</button>
                </div>
                <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pb-4">
                   {game.events.slice().reverse().map((e) => (
                    <div key={e.id} className="bg-surface/60 border border-surfaceVariant p-4 rounded-2xl flex items-center justify-between">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar">
                  <div className="space-y-4">
                    {[
                      ...
                      { l1: 'Faltas', v1: getStat(['FALTA']), c1: 'text-red-600', l2: 'Tarjetas', v2: getStat(['VERDE', 'AMARILLA', 'ROJA']), c2: 'text-yellow-600' },
                      { l1: 'C. Corto', v1: getStat(['CORNER CORTO']), c1: 'text-indigo-600', l2: 'Penales', v2: getStat(['PENAL']), c2: 'text-purple-600' },
                    ].map((row, idx) => (
                    ...
                    {showPopup.type === 'FOUL' ? (
                      <>
                        <div className="grid grid-cols-2 gap-2">
                           <button className="text-[10px] font-black text-onSurface py-3 rounded-xl bg-surface border border-surfaceVariant flex items-center justify-center gap-2" onClick={() => registerEvent('FALTA (VERDE)', Possession.AWAY, showPopup.x, showPopup.y, 'VERDE', game.teamHome.id)}>
                             <span className="w-3 h-3 bg-green-500 rounded-sm"></span> VERDE
                           </button>
                           <button className="text-[10px] font-black text-onSurface py-3 rounded-xl bg-surface border border-surfaceVariant flex items-center justify-center gap-2" onClick={() => registerEvent('FALTA (AMARILLA)', Possession.AWAY, showPopup.x, showPopup.y, 'AMARILLA', game.teamHome.id)}>
                             <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span> AMAR
                           </button>
                        </div>
                        <button className="text-[10px] font-black text-indigo-600 py-3 rounded-xl bg-indigo-50 border border-indigo-100 uppercase" onClick={() => registerEvent('CORNER CORTO', Possession.NONE, showPopup.x, showPopup.y, "C. CORTO", game.teamHome.id)}>🏑 CORNER CORTO</button>
                        <button className="text-[10px] font-black text-purple-600 py-3 rounded-xl bg-purple-50 border border-purple-100 uppercase" onClick={() => registerEvent('PENAL', Possession.NONE, showPopup.x, showPopup.y, "PENAL", game.teamHome.id)}>🎯 PENAL</button>
                      </>
                    ) : (
                      <>
                        <button className="text-xs font-black text-primary text-left py-4 px-5 rounded-xl bg-primary/5 border border-primary/10 flex items-center gap-4 uppercase" onClick={() => registerEvent('DISPARO (GOL)', showPopup.targetGoal === 'TOP' ? Possession.AWAY : Possession.HOME, showPopup.x, showPopup.y, "GOL", game.teamHome.id, showPopup.targetGoal === 'TOP' ? Possession.HOME : Possession.AWAY)}>⚽ GOL</button>
                        <button className="text-xs font-black text-dark text-left py-4 px-5 rounded-xl bg-surface border border-surfaceVariant flex items-center gap-4 uppercase" onClick={() => registerEvent('DISPARO (ATAJADO)', Possession.NONE, showPopup.x, showPopup.y, "ATAJADO", game.teamHome.id)}>🛡️ ATAJADO</button>
                        <button className="text-xs font-black text-onSurfaceVariant text-left py-4 px-5 rounded-xl bg-surface border border-surfaceVariant flex items-center gap-4 uppercase" onClick={() => registerEvent('DISPARO (DESVIADO)', Possession.AWAY, showPopup.x, showPopup.y, "DESVIADO", game.teamHome.id)}>💨 DESVIADO</button>
                      </>
                    )}
          ...
          {undoModal && (
            <div className="absolute bottom-16 left-0 bg-white border border-surfaceVariant p-5 rounded-[28px] shadow-2xl w-52 animate-in slide-in-from-bottom duration-200 z-[200]">
              ...
              <div className="flex gap-2.5">
                <button onClick={() => { deleteEvent(game.events[game.events.length-1].id); setUndoModal(false); }} className="flex-1 bg-red-600 text-white text-[9px] font-black py-3 rounded-xl uppercase">BORRAR</button>
